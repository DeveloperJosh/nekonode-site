...
const EpisodePlayer = ({ episode, setSelectedServer, selectedServer }) => {
  console.log('EpisodePlayer:', episode);
  const [loading, setLoading] = useState(true);
  const [playerUrl, setPlayerUrl] = useState('');
  const [episodeLoaded, setEpisodeLoaded] = useState(false);

  useEffect(() => {
    if (!episode || !episode.episodeNumber || !episode.animeId) {
      setPlayerUrl('');
      setLoading(false);
      return;
    }
  ...
  useEffect(() => {
    if (episodeLoaded) {
      // Construct the episode identifier
      const encodedAnimeId = encodeURIComponent(episode.animeId);
      const episodeId = `${encodedAnimeId}-episode-${episode.episodeNumber}`;

      // Build the player URL
      const url = `${player_url}?anime_id=${episodeId}&server=${selectedServer}`;
        ...
        {!loading && !playerUrl && (
          <div className="absolute inset-0 flex items-center justify-center bg-black">
            <p className="text-red-500">
              No suitable source found for the selected server.
            </p>
          </div>
        )}
      </div>
