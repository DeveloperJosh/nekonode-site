...
const UpdateToList = ({ isOpen, onClose, anime, onUpdate }) => {
  const [animeStatus, setAnimeStatus] = useState(anime.status);
  const [error, setError] = useState('');

  const handleSubmit = () => {
    setError('');
    onUpdate(anime.animeId, animeStatus);
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-xl font-bold mb-4 text-yellow-500">Update Anime Status</h2>
