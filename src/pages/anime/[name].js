...
const AnimePage = () => {
  ...
  const { data: animeData, error } = useSWR(
    name ? `${api}/anime/${name}` : null,
    fetcher,
    {
  ...
  console.log('Anime Data:', selectedEpisode);

  useEffect(() => {
    if (name && animeData) {
      const firstEpisodeNumber = animeData.episodes[0];
      const episodeToPlay = ep
        ? parseInt(ep, 10)
        : firstEpisodeNumber;
    ...
    return (
      <div className="text-center">
        ...
        <p className="text-zinc-600 dark:text-zinc-400">
          Your Anime adventure is about to begin
        </p>
      </div>
    );
  }
  ...
  const episodeButtons = currentEpisodes.map((episodeNumber) => (
    <li key={episodeNumber} className="mb-2">
      <button
        className={`w-full text-left px-4 py-2 rounded ${
          selectedEpisode && selectedEpisode.episodeNumber === episodeNumber
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="flex flex-col md:flex-row">
            <aside className="md:w-1/4 bg-gray-800 p-4 rounded-lg shadow-lg mb-4 md:mb-0 flex flex-col h-full">
              <div
                className={`flex-1 overflow-y-auto ${
                  episodes.length > 1 ? 'max-h-96' : 'max-h-auto'
                }`}
              >
                ...
                <div className="block md:hidden">
                  <select
                    ...
                    onChange={(e) =>
                      handleEpisodeSelect(parseInt(e.target.value, 10))
                    }
                    value={
                ...
                <ul className="hidden md:block">{episodeButtons}</ul>
              </div>
            ...
            <main className="w-full md:w-3/4 md:ml-4">
              ...
              <AnimeDetails
                animeInfo={animeInfo}
                onAddToListClick={() => setIsModalOpen(true)}
                isAuthenticated={!!session}
              />
              <Comments
                animeId={name}
                episodeNumber={
                  selectedEpisode ? selectedEpisode.episodeNumber : null
                }
              />
              {session && (
                <CommentForm
...
const AnimeDetails = ({
  ...
  <div className="bg-gray-800 p-6 rounded-lg shadow-lg flex flex-col md:flex-row items-center">
    ...
    <div className="text-gray-300 mt-4 md:mt-0 md:ml-4">
      <h1 className="text-4xl font-bold text-left mb-4 text-yellow-500">
        {animeInfo.title}
      </h1>
      <p className="mb-4">
      ...
      <p className="mb-2">
        <span className="font-semibold">Status:</span> {animeInfo.status}
      </p>
      <p className="mb-2">
